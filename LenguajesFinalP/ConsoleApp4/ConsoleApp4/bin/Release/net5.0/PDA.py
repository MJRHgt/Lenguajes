...
class PDA:
    ...
    def compute(self, inputString, parsedLines):
        ...
        print('Estado\tEntrada\tPila\tMovimiento')
        ...
        for char in inputString:
            ...
            for production in productions:
                if ((production[0] == currentState) and (production[1] == char) and (production[2] == currentStackSymbol)):
                    currentState = production[4]
                    if(len(production[3]) == 2):
                        self.stack.append(char)
                    elif(len(production[3]) == 3):
                        self.stack.append(char)
                        self.stack.append(char)
                    elif ((production[3] == 'e') and (len(self.stack) != 1)):
                        self.stack.pop()
                        break
